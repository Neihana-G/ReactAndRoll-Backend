...
module.exports = {
    signinStudent: async (req, res) => {
        ...
        if(!result.length){
            ...
            const token = jwt.sign({ student_id, userState: 'student' }, process.env.SECRET_KEY, {
                expiresIn: 1500,
            });
            ...
            bcrypt
                ...
                .then(function (result) {
                    if (result) {
                        ...
                        res.status(401).send({ message: "Invalid log in credentials" });
                    }
                })
                .catch((err) => {
    ...
    signinTeacher: async (req, res) => {
        ...
        if(!result.length){
            ...
            const token = jwt.sign({ teacher_id, userState: 'teacher' }, process.env.SECRET_KEY, {
                expiresIn: 1500,
            });
