...
module.exports = {
  ...
  loginTeacher: async (email) => {
    const result = await pool
      .promise()
      .execute("SELECT teacher_id, password FROM teacher WHERE email = ?", [
        email,
      ])
      .then(([result]) => {
  ...
  loadStudent: async (id) => {
    const result = await pool
      .promise()
      .execute(
        "SELECT student_id, name, email, school, profile_pic, date_of_birth, contact_number, course, teacher_id FROM studentprofile WHERE student_id = ?",
        [id]
      )
      .then(([result]) => {
        return result;
